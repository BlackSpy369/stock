...
if ticker==" ":
    st.warning("Please select a Stock to get Started, either load your own data or use inbuild datasets of stocks.")

if ticker!=" ":
    metric=st.selectbox('Select a Metric:', df.columns.insert(0," "), format_func=lambda x: 'Select an option' if x == '' else x)
    ...
    if metric!=" ":
        if uploaded_file or ticker: 
            with st.status("Running ...", expanded=True) as status:
                ...
                time.sleep(sleep_time)
                X,y=preprocess_data(df[metric].values,n_length)
                ...
                time.sleep(sleep_time)
                performance_dict={"Train MSE":train_mse,"Train R2":train_r2,"Test MSE":test_mse,"Test R2":test_r2}
